...
{
    //using gpio pin power oled - this way it can be reset
    gpio_init(I2C_POWER);
    gpio_set_dir(I2C_POWER, GPIO_OUT);
    gpio_put(I2C_POWER, 0);
    sleep_ms(10);
    gpio_put(I2C_POWER, 1);
    sleep_ms(10);
    ...
    i2c_write_blocking( i2c_default, 0x3c, data, 26, false);
    return(0);
}
...
static int ssd1306_blank(uint8_t val)
{
   ...
   i2c_write_blocking( i2c_default, 0x3c, data, len/2, false);
   free(data);
   return(0);
}
...
static int ssd1306_text( char *disp_string )
{
   ...
   uint8_t *data = malloc (len+1);
   data[0] = 0x40;
   for(int i=0; i<len+1; i++){ data[i+1] = framebuffer[i]; }

   i2c_write_blocking( i2c_default, 0x3c, data, len/2+1, false);
   free(framebuffer);
   free(data);
   return(0);
}
